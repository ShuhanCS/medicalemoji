...
| --- | --- | --- | --- |
| Kidney | [`kidney-agent-spec.md`](kidney-agent-spec.md) | `agent/kidney-2026` | `v1.9.0-kidney.1` |
...
for example `v1.9.0-kidney.2`. An agent never edits an earlier lane snapshot.
...
Each agent copies the complete `submissions/v1.8.0/` folder to its assigned prerelease folder. Pill Pack also
imports its complete historical concept folder from `submissions/v1.3.0/pill-pack/`. Agents may change only
their unique prerelease folder. They must not change README files, the controlling slate specification, root
`CHANGELOG.md`, `package.json`, `package-lock.json`, another lane, or canonical package `v1.8.0`.
...
1. Kidney becomes canonical `v1.9.0`.
2. White Blood Cell is applied on top of `v1.9.0` and becomes `v1.10.0`.
3. Stomach is applied on top of `v1.10.0` and becomes `v1.11.0`.
4. Liver is applied on top of `v1.11.0` and becomes `v1.12.0`.
5. Pill Pack, only if advanced, is applied on top of `v1.12.0` and becomes `v1.13.0`.
