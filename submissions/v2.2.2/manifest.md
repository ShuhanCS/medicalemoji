...
PATCH bump: submitter ordering only; the submitter set, proposal argument, and artwork are unchanged.

- Reordered the submitter line to the requested final sequence.
- Placed Lauren Beaudin immediately after Timur Erk.
- Placed Heena Purohit before David Rhew.
- Regenerated the review PDF.

## What changed from v2.2.0

PATCH bump: submitter roster update only; proposal argument and artwork are unchanged.

- Added Lauren Beaudin, David Rhew, and Heena Purohit to the submitter line.
- Removed Jarone Lee from the submitter line.
- Updated the proposal date to 2026-07-28 and regenerated the review PDF.

## What changed from v2.1.2

MINOR bump: substantive case change, no artwork change.

- `C. Breaks new ground` now leads with a verified defect in the standard rather than with absence.
  In CLDR English annotation data, `kidney` is a keyword on exactly one emoji, U+1FAD8 BEANS, a
  Food & Drink pictograph. Every emoji picker and in-app search consuming CLDR inherits that
  routing, so a user searching for an organ is shown a legume.
- `G. Compatibility` no longer answers `Not applicable`. It states the CLDR routing problem and
  requests no annotation change, since keyword assignment is a CLDR matter. ESR's stated Emoji 19.0
  intake priorities (L2/25-128, reconfirmed L2/26-098) name compatibility with apps, standards, and
  operating systems as a focus, which made `Not applicable` the costliest possible answer.
- `A. Already representable` now distinguishes being reachable by a search term from representing
  the concept.
- Artwork is byte-identical to v2.1.2. Validation rerun and passing.
- Prose 1,188 words against the 1,200 ceiling.

## What v2.1.2 changed from v2.1.1
...
| --- | --- | --- |
| `v2.2.2_kidney_emoji_proposal_SUBMIT.md` | `SUBMIT` | PDF-facing proposal source. |
| `v2.2.2_kidney_emoji_proposal_SUBMIT.pdf` | `SUBMIT` | Rendered PDF. Not yet hosted at a public URL. |
...
| `v2.2.2_support_letters_REFERENCE_ONLY.md` | `REFERENCE_ONLY` | The 13 institutional support letters, retained but kept out of the proposal. |
