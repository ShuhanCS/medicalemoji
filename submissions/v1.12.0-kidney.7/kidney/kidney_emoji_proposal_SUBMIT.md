...
**Submitter:** Shuhan He; Edgar Lerma; Caitlyn Vlasschaert; Jade M. Teakell; Harish Seethapathy; Jarone Lee; Danielle Miller; Timur Erk; David Rhew, MD (Microsoft); Heena Purohit (Microsoft for Startups)<br>
...
**Suggested keywords:** renal; anatomy; organ; filtration; urine; dialysis; transplant; donation; stone; kidney test<br>
...
The most direct evidence of expected usage is that people already send a kidney pictograph. Lacking a Kidney
character, writers substitute Beans: third-party emoji reference sites index Beans combinations under kidney
and nephrology headings, and practitioners report that the substitution is confusing in the context of kidney
disease. That is demand expressed pictographically rather than inferred from word counts, and it is the exhibit
this factor rests on first. See [Kidney emoji combinations](https://emojicombos.com/kidney) and
...
compare it with `elephant`. They are offered as supporting context rather than as the primary case. The
unfiltered term also captures `kidney bean` and disease-related pages, so these counts establish sustained term
visibility only, not medical importance or guaranteed emoji use.
...
keyword makes it searchable, and writers who need the organ reach for it in the absence of anything better.
...
shows the substitute does not meet it. A Kidney character supplies the missing noun. See the
[Unicode Emoji List](https://unicode.org/emoji/charts/emoji-list.html).
...
The closest unencoded neighbors are Liver, Stomach, Pancreas, and Spleen. Kidney has a specific combination that
those names cannot simply reuse: kidney stone and dialysis, together with kidney testing and donation/transplant
language. Beans and short sequences still fail to identify the organ, the required frequency exhibits show
sustained use of its name, and the paired notched form remains legible at small size. Kidney is not proposed
because organs are important or because an anatomy set should be completed; encoding it would not establish
that every neighboring organ meets the same evidence.
