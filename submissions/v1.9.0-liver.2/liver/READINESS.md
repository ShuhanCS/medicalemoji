# Liver v1.9.0-liver.2 Readiness Report

Review date: 2026-07-24

Status: **REVISION REQUIRED**

Internal readiness score: **83/100**
...
- Package: `submissions/v1.9.0-liver.2/`
- Sole submitter and main point of contact: Shuhan He
- Canonical source package: `submissions/v1.9.0-liver.1/`
- Scope: only this prerelease's `VERSION`, `manifest.md`, `CHANGELOG.md`, and `liver/` differ from
  `v1.9.0-liver.1`.

## Decision

The source, artwork, visual-comparison evidence, complete current frequency evidence, and PDF are materially
improved. The package is not ready to publish because human recognition, confirmation of the revised 18x18 art
...
| Frequency evidence | **Pass** | All five required sources are current. Google Search and Video result-stat values were captured 2026-07-24 and transparently revealed from Google's native, zero-layout `#result-stats` element. |
...
| Frequency and empirical evidence | 20 | 20 | All five sources are current; Search and Video values are preserved with an explicit zero-layout DOM disclosure. |
...
| **Total** | **100** | **83** | **Revision required.** |

## Evidence record

| Source | Date and settings | Result |
| --- | --- | --- |
| Google Search | 2026-07-24; Google Web results; query shown below | Native `#result-stats`: `About 194,000 results (0.26s)`. Google collapses that element to zero layout; the evidence image labels the exact value it supplied. |
| Google Video Search | 2026-07-24; Google Video results; query shown below | Native `#result-stats`: `About 82,000,000 results (0.38s)`. Google collapses that element to zero layout; the evidence image labels the exact value it supplied. |
...
Full Search URL:
https://www.google.com/search?udm=14&q=liver&hl=en&num=10&pws=0
...
- Copied all 77 v1.9.0-liver.1 files before editing; initial copy had zero SHA-256 mismatches.
...
- Rebuilt with `python scripts/make_submission_pdf.py submissions/v1.9.0-liver.2/liver/liver_emoji_proposal_SUBMIT.md`.
- Verified the PDF is 10 letter-size pages, 634,539 bytes, tagged, unencrypted, and free of forms and JavaScript.
...
- Verified all local Markdown image links resolve and the PDF contains the four required example images, two
  embedded visual-comparison boards, two current Google evidence captures, and three remaining frequency
