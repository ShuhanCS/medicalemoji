...
**Suggested keywords:** renal; dialysis; transplant; donation; stone<br>
...
| --- | --- | --- |
| 18x18 | ![Kidney color 18x18](images/v2.10.0_kidney_color_18x18_SUBMIT.png) | ![Kidney black-and-white 18x18](images/v2.10.0_kidney_bw_18x18_SUBMIT.png) |
| 72x72 | ![Kidney color 72x72](images/v2.10.0_kidney_color_72x72_SUBMIT.png) | ![Kidney black-and-white 72x72](images/v2.10.0_kidney_bw_72x72_SUBMIT.png) |
...
polished 72-pixel example adds restrained shading and detailed renal vessels; the purpose-built 18-pixel example
uses the same paired notched contour with a simplified central connector and two short ureter cues. Those cues
remain visible at the required native size and distinguish the image from separate Beans and from the symmetric
lobes and top-entering airway of Lungs. Vendors may vary color, shading, and fine anatomical detail while
preserving the paired notched bodies and their central anatomical connection.

### E. Expected usage level

The five required frequency methods provide this proposal's primary empirical evidence of expected usage.
They test the exact proposed name, `kidney`, against `elephant` in Google Search, Google Video Search, Google
Trends Web Search, Google Trends Image Search, and Google Books Ngram Viewer. Search and Video use separate
query exhibits for the two terms; Trends and Books display both terms in the same exhibit. Together, the five
...
Kidney was captured 2026-07-28 in signed-out Google Web Search in Firefox Private Browsing, with
English-language results and no date or country filter selected. The page reports about 211,000,000 indexed
results.
[Open the reproducible Kidney Google Search query](https://www.google.com/search?q=kidney&hl=en&num=10&pws=0).

![Google Search results for kidney](evidence/frequency/v2.10.0_kidney_google_search_2026-07-28_SUBMIT.png)

Elephant was captured immediately afterward in the same Firefox Private Browsing session with the same
signed-out search type, language, and no date or country filter selected. The page reports about 487,000,000
indexed results.
[Open the reproducible Elephant Google Search query](https://www.google.com/search?q=elephant&hl=en&num=10&pws=0).

![Google Search results for elephant](evidence/frequency/v2.10.0_elephant_google_search_2026-07-28_SUBMIT.png)

Google's approximate index counts are volatile snapshots, but the required comparison is clear: Kidney has
about 211 million results against Elephant's 487 million, placing the proposed name at roughly 43% of the
comparison term in this source.

#### Google Video Search

Kidney was captured 2026-07-28 in signed-out Google Video Search in the same Firefox Private Browsing session,
with English-language results and no date or country filter selected. The page reports about 53,600,000 indexed
results.
[Open the reproducible Kidney Google Video Search query](https://www.google.com/search?tbm=vid&q=kidney&hl=en&num=10&pws=0).

![Google Video Search results for kidney](evidence/frequency/v2.10.0_kidney_google_video_search_2026-07-28_SUBMIT.png)

Elephant was captured immediately afterward with the same signed-out video-search type, language, and no date
or country filter selected. The page reports about 85,900,000 indexed results.
[Open the reproducible Elephant Google Video Search query](https://www.google.com/search?tbm=vid&q=elephant&hl=en&num=10&pws=0).

![Google Video Search results for elephant](evidence/frequency/v2.10.0_elephant_google_video_search_2026-07-28_SUBMIT.png)

In the required Video comparison, Kidney has about 53.6 million results against Elephant's 85.9 million, or
roughly 62% of the comparison term. These counts are used only as the form's requested relative index measure,
not as a claim that raw result totals measure importance.
...
![Google Trends Web Search for elephant and kidney](evidence/frequency/v2.10.0_kidney_google_trends_web_elephant_2026-05-13_SUBMIT.png)
...
![Google Trends Image Search for elephant and kidney](evidence/frequency/v2.10.0_kidney_google_trends_image_elephant_2026-05-13_SUBMIT.png)
...
![Google Books Ngram for elephant and kidney](evidence/frequency/v2.10.0_kidney_google_books_ngram_elephant_2026-07-09_SUBMIT.png)
...
fine anatomy while preserving two inward-facing notched bodies and a visible offset. The 18x18 examples retain
only a simplified central connector and short ureter cues; detailed vascular anatomy remains in the 72x72
examples.
