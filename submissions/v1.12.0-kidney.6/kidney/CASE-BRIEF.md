...
Kidney satisfies all four in the `.6` candidate. The strongest polished GPT Image 2 concept now supplies the
72x72 examples, while the purpose-built 18x18 pair preserves stronger small-size separation and passes the
fixed technical checks.
...
| Distinctiveness | `SUPPORTED` | Paired inward-facing forms, medial notches, diagonal offset, and short central attachment are stable cues; the `.6` combined color and black-and-white assets pass every fixed comparator check. |
...
### Accepted-proposal calibration

Accepted proposals vary substantially in sequence count: Treasure Chest gives several thematic combinations,
X-Ray gives three broad combinations, and Fingerprint gives a longer technical list. The current rule sets no
minimum and asks only for compelling examples. Kidney therefore keeps its two clearest, source-backed phrases:
Kidney + Rock for `kidney stone` and Kidney + Test Tube for `kidney tests`. Donation/transplant combinations
were rejected here because they need more explanation than the resulting sequence.

Expected usage is also kept separate from medical importance. The public paragraph now names ordinary
communication situations in plain language, while the five required exhibits carry the frequency argument.
Comparator sources: https://www.unicode.org/L2/L2024/24255-treasure-chest-emoji.pdf,
https://www.unicode.org/L2/L2020/20214-x-ray-emoji.pdf, and
...
cluster. The raw-word and Image Trends weaknesses are disclosed. The `.6` set combines the polished 72px GPT
Image 2 masters with the `.5` purpose-built 18px pair, which keeps color Lungs IoU at `0.698` under the unchanged
...
submission-ready: eligibility documentation, duplicate coordination, Shuhan's approval of the exact `.6`
assets, exact-hash panel review, publication, and form reconciliation remain separate must-pass gates.

## Phase 4 - Artifact update

The `.6` 72x72 color and black-and-white examples are exports from the strongest polished GPT Image 2 masters.
The purpose-built 18x18 assets remain byte-identical to `.5` because a mechanical reduction of the detailed
master weakened separation from Lungs. The combined set passes dimensions, true black-and-white palette,
connectedness, IoU, and difference-hash checks. This is technical evidence only and does not claim human
semantic recognition.

| Current `.6` asset | SHA-256 |
| --- | --- |
| `images/kidney_color_18x18_SUBMIT.png` | `619ec36cecf9a0637789a0a1ebbc3dbc2443c964405ccf395ea7f35fc30f950e` |
| `images/kidney_color_72x72_SUBMIT.png` | `f671831a59a008f0d32a3796417d45ddc39a4f613bdebac3d4f5078adf461c0c` |
| `images/kidney_bw_18x18_SUBMIT.png` | `b006dc320958ff14bf1fc9024644dc7de8d199588887ffa63dd9f17a20e70d5b` |
| `images/kidney_bw_72x72_SUBMIT.png` | `fd6fbbbf41dfda556fad0420174a7da87f26db006faccd234e07f842f3023ea7` |
...
| Use in sequences | Two concise evidenced combinations—Kidney + Rock for `kidney stone` and Kidney + Test Tube for `kidney tests`; no weaker padding | `PASS` |
...
| Expected usage | Plain-language stone, test, dialysis, transplant, and donation contexts plus five interpreted captures | `PASS` |
