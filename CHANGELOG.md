...
### Added
- Recorded Shuhan He's dated approval of the exact four Stomach artwork files, tied to their dimensions and
  SHA-256 hashes.
- Recorded Shuhan He's confirmation that David Rhew and Heena Purohit consented to the exact ordered Stomach
  byline, while keeping private correspondence outside the public repository.

### Changed
- Advanced the Stomach control record to `1.13.0-candidate.3`; the proposal, evidence, artwork, and verified PDF
  remain byte-identical to Candidate.2.
- Advanced the workspace version from `0.49.2` to `0.49.3`.
...
### Changed
- Narrowed the 2026 filing slate to two independent proposals, Stomach and Kidney, after Maze was confirmed
  ineligible for this cycle.
- Removed Maze from the Stomach/Kidney same-cycle execution strategy and recorded it as a 2026 no-submit.
- Advanced the workspace version from `0.49.0` to `0.49.1`.
...
### Changed
- Added the complete, visible GitHub URL for the shared proposal-building instructions to the main README.

## [0.47.1] - 2026-07-26

### Changed
- Removed defensive and self-litigation framing from the shared proposal instructions. Agents now present a
  compelling supported case and answer required exclusion factors briefly in affirmative proposal language.
- Advanced the shared guidance and four active agent specifications by one patch version and the workspace to
  `0.47.1`.
...
- Recorded `STOP THIS CYCLE` for the frozen artifact while the four-year eligibility conflict remains open;
  ...
  artwork corrections.
- Advanced the workspace version from `0.46.1` to `0.46.2`. The immutable Kidney artifact remains
...
### Added
- Added a shared case-building and drafting specification for the Stomach, Kidney, Liver, and White Blood Cell
  agents, with required case briefs, claim ledgers, anti-generic gates, section ownership, and adversarial
  review before compliance scoring.

### Changed
- Made the rubric and template downstream compliance checks instead of prose-generation systems in the four
  active agent specifications.
- Advanced the agent coordination index to `1.4.0`, Kidney specification to `1.4.0`, the other three active
  specifications to `1.2.0`, and the workspace to `0.46.0`.
...
- Created the immutable `1.12.0-liver.5` submission package and synchronized rubric `3.2.1`.
...
### Fixed
- Added an identifying running footer with the paper title, authors, and page number to every page of the
  final health-related emoji coverage UTC submission, while keeping the upper-right corner clear for
  Unicode's assigned document number.
- Wrapped the requested-action field within the document margins and rebuilt the PDF with LuaLaTeX so its
  embedded Georgia text extracts with the correct Unicode punctuation.

### Verification
- Confirmed the three-page PDF renders without clipping or overlap, includes the identifying footer on every
  page, embeds every font, preserves all source links, and contains no encryption or provisional L2 number.
- Advanced the project version from `0.29.0` to `0.29.1`; the proposal release remains `v1.7.0` because no
  individual emoji proposal changed.
...
### Added
- Added a formatted DOCX version of the ACEP medical emoji memo with citations on a second page.

## [0.19.10] - 2026-05-15

### Changed
- Simplified the ACEP memo ask by replacing process terms with plain-language leadership deliverables.
...
### Changed
- Clarified that the JAMA medical emoji article's He citation refers to Shuhan He, MD, the ACEP Informatics Section Chair authoring the memo.

## [0.19.6] - 2026-05-15

### Changed
- Condensed the ACEP medical emoji memo into a one-page executive memo with citations and links on a second page.
...
### Changed
- Cleaned contrastive "not X / it is Y" wording from the ACEP memo for a more direct leadership tone.
...
### Changed
- Updated the ACEP medical emoji memo with JAMA citations, kidney and stomach supporter counts, and 2026 kidney/stomach submission coordination language.
...
### Added
- Added a kidney `v0.10.0` improvement plan based on accepted Unicode proposal analysis.

## [0.13.0] - 2026-05-13

### Added
- Added a Unicode accepted emoji proposal manifest and analysis of successful proposal patterns for kidney/stomach planning.
...
### Added
- Added kidney emoji packet `v0.7.0` as a planned submission candidate with `SUBMIT` proposal/image files and a `SIGNATURE_REQUIRED` finalization control.

## [0.8.1] - 2026-05-13

### Added
- Added kidney emoji packet `v0.6.1` with explicit Unicode submission form URLs and final public-PDF routing note.
